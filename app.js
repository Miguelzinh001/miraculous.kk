...
function renderAdminAnnouncements(){
  ...
  $('adminAnnouncements').innerHTML=announcements.map(a=>`<div class="admin-item"><div><b>${esc(a.title)}</b><br><span class="muted">${esc(a.label || 'NOVIDADE')}</span></div><button class="delete-announcement" data-id="${esc(a.id)}">Apagar</button></div>`).join('');
...
async function addAnnouncementFromForm(){
  const titleEl=$('announcementTitle')||$('annTitle')||$('newsTitle');
  const bodyEl=$('announcementText')||$('annText')||$('newsText');
  const labelEl=$('announcementLabel')||$('annLabel')||$('newsLabel');
  const colorEl=$('announcementColor')||$('annColor')||$('newsColor');
  const title=titleEl?.value.trim() || bodyEl?.value.trim();
  const body=bodyEl?.value.trim() || title;
  const label=labelEl?.value.trim() || 'NOVIDADE';
  const label_color=colorEl?.value || '';
  if(!title) return false;
  const {data:{user}}=await supabase.auth.getUser();
  if(!user) return alert('Inicia sessão como administrador primeiro.');
  const payload={title,body,label,label_color,published:true};
